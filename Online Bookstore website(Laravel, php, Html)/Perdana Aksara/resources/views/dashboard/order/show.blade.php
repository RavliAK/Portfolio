       ...
       @if ($order->pmethod === 'credit' || $order->pmethod === 'debit')
        ...
        <form action="/dashboard/order/{{$order->id}}" method="post" class="d-inline">
          ...
          <select class="form-select" name="status" id="status" >
            <option value="">Current Status: {{ $order->status }}</option>
            <option>Done</option>
          ...
          <button type = "submit" class="btn btn-outline-primary btn-sm btn-icon">
            <i data-feather="update"></i>Update status
            </button>
            <a class="nav-link" href="/email"><i class="bi bi-mail"></i> Send invoice to user</a>
          </div>
        </form>
       @endif
